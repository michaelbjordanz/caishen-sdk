...
import { BaseRequest, SignAndSendPayload, SignPayload, WalletAccount } from '../types';
...
/**
 * Signs without broadcasting to the network.
 * @param request
 */
export async function sign(
  this: CaishenSDK,
  request: BaseRequest<WalletAccount, SignPayload>,
): Promise<string> {
  const {
    wallet,
    payload,
  ...
  const url = `${BASE_URL}/api/crypto/sign`;
  const response = await axios.post(
    url,
    {
      wallet,
